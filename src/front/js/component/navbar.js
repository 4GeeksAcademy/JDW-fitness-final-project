...
export const Navbar = () => {
    ...
    useEffect(() => {
        const checkPendingRequests = async () => {
            if (loggedCoach) {
                ...
                const pendingRequests = store.receivedLikesCoach.filter(
                    (like) => !store.matches.some((match) => match.client_id === like.client_id && match.coach_id === loggedCoach.id)
                );
    ...
    }, [loggedCoach, store.receivedLikesCoach, store.matches]);//AQUI ESTA EL BUCLE
				...
				<div className="d-flex align-items-center ms-auto me-4">
			  	<div className="nav-item dropdown notification_dropdown me-5">
                                <a className="nav-link ai-icon-notification rounded-3" role="button" data-bs-toggle="dropdown">
									...
									{nullElement || hasPendingRequest && <div className="pulse-css"></div>}
                                </a>
                                <div className="dropdown-menu rounded dropdown-menu-end">
                                    <div id="DZ_W_Notification1" className={`widget-media dz-scroll-notifications pe-1 ps-3 py-3 ${!nullElement && notifications.length == 0 ? 'height-no-notifications0' : 'height38'}`}>
											{!nullElement && !hasPendingRequest &&	
										...
										<ul className="timeline ps-1">
											...
											<li>
												<div className="timeline-panel">
													...
													<div className="media-body">
														...
														<Link to={`/${currentUser}/${currentUserID}`} ><i class="fa-solid fa-user-pen fs-5 ms-2 text-secondary"></i></Link>
														</h6>
											...
											<li>
												<div className="timeline-panel">
													...
													<div className="media-body">
														<h6 className="mb-1">You have pending request
														<Link to={`/${currentUser}/likes/received`} ><i className="fa-solid fa-user-clock fs-5 ms-2 text-secondary"></i></Link>
														</h6>
													</div>
												</div>
											</li>
